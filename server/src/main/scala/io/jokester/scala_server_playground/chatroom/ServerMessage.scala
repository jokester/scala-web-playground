...
object ServerMessage {
  ...
  case class JoinedChannel(seq: Int,
                           channel: Channel,
                           users: Seq[User],
  ...
  case class ChannelBroadcast(channel: Channel,
                              joinedUsers: Seq[User],
                              leftUsers: Seq[User],
                              newMessages: Seq[ChatMessage])

  case class BroadCast(seq: Int,
                       channels: Seq[ChannelBroadcast],
                       newUsers: Seq[User])
      extends ToUser(seq)
